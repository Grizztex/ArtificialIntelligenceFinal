...
SEE ALSO
    ...
    ##validate data in distance objects
    if names == '':
...
##retrieving model and id correlated to that coordinate
    """ original code cannot work with protein with more 2 chains
    xyz2idx = {}
    cmd.iterate_state(state, selection, 'xyz2idx[x,y,z] = (model,index)',
                      space=locals())
    """
    ##new code including chain
    xyz2idx = {}
    cmd.iterate_state(state, selection, 'xyz2idx[x,y,z] = (model,index,chain)',
                      space=locals())
